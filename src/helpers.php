...
if (! function_exists('fileFormat')) {
     ...
     * @return string
     */
    function fileFormat($bytes): string
    {
...
if (! function_exists('countViews')) {
    ...
    {
        $label = ['view', 'views', 'K views', 'M views', 'B views', 'T views'];

        for ($i = 0; $views >= 1000 && $i < (count($label) - 1); $i++) {
            $views /= 1000;
        }

        return round($views, 2).' '.$label[$i];
    }
}
